...
The app is built as a small showcase of recent iOS app integration points. The current source keeps these APIs behind availability checks so the catalog still runs on older deployment targets when the newer frameworks are unavailable.
...
- The Xcode project deployment target is iOS 17, with newer API paths guarded by `#available` and `#if canImport(...)`.
